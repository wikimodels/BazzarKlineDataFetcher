...
(АСИНХРОННАЯ ВЕРСIA) - для соответствия cache_manager.py и config.py
"""
import logging
...
import pytz
import html # <--- ИСПРАВЛЕНИЕ: Добавлен 'html' для экранирования
...
def _format_report_time() -> str:
    ...
    # --- 🚀 ИСПРАВЛЕНИЕ (NameError): 'time_str' определен до return ---
...
async def send_triggered_alerts_report(
    ...
    if not alerts:
        ...
        for i, alert in enumerate(alerts):
            ...
            # --- 🚀 ИСПРАВЛЕНИЕ (AttributeError): httpx.utils -> html.escape ---
