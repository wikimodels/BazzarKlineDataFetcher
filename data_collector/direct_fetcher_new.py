...
# --- ДОБАВЛЕНО: Исправление пути для импорта из корня ---
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))

# --- Импорты из Проекта (Относительные/Абсолютные) ---
...
async def run_direct_data_collection(timeframe: str, symbols: Optional[List[str]]) -> Response:
    ...
    try:
        ...
        from .coin_source import get_coins as get_all_symbols
        from .. import config
    ...
    try:
        ...
        # --- ИСПРАВЛЕНИЕ GZIP: Убираем Content-Encoding ---
        return Response(
